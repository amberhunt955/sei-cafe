...
export default function OrderDetail({ order, handleChangeQty, handleCheckout }) {
  ...
  const lineItems = order.lineItems.map((item) => (
    <LineItem lineItem={item} isPaid={order.isPaid} handleChangeQty={handleChangeQty} key={item._id} />
  ));
          ...
          <>
            ...
            <section className={styles.total}>
              {order.isPaid ? (
                ...
                <button className="btn-sm" onClick={handleCheckout} disabled={!lineItems.length}>
                  CHECKOUT
                </button>
              )}
